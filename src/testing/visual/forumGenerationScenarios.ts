...
export async function applyForumGenerationVisualScenario(name: string, context: ForumGenerationScenarioContext) {
  ...
  if (name === 'forum-catalog') {
    ...
    if (toolbar?.textContent?.includes(longTypePrompt)) throw new Error('Long forum type prompt leaked into the visible board header');
    if (topTitle?.textContent?.trim() !== board.name) throw new Error('Forum board header did not preserve the board name');
    if (toolbar?.textContent?.includes('视觉自定义类型')) throw new Error('Forum board header still exposes the removed type label');
  } else if (name === 'forum-board-editor' || name === 'forum-bagu' || name === 'forum-thread' || name === 'forum-thread-editor') {
    const { board, thread } = createForumFixture();
    const page = name === 'forum-board-editor' ? 'board-editor' : name === 'forum-bagu' ? 'bagu-scan' : name === 'forum-thread' ? 'thread' : 'thread-editor';
    const title = name === 'forum-board-editor' ? '编辑板块' : name === 'forum-bagu' ? '八股检测' : name === 'forum-thread' ? thread.title : '编辑帖子';
    context.resetPhoneToRoute('forum', page, title, name === 'forum-board-editor' ? { boardId: board.id } : { boardId: board.id, threadId: thread.id });
  } else if (name === 'forum-failed-draft') {
    forum.resetCurrentScope();
    const draft = forum.createFailedDraft({
      actionId: 'generate-thread', appId: 'forum', context: {},
      rawOutput: '<forum><title>未闭合</title><content>等待修复的主题帖',
      source: visualSource('visual-forum-chat', 3, 15), warnings: ['缺少结束标签'],
    });
    ...
    usePreviewDraftStore().upsertPreviewDraft({
      appId: 'forum', page: 'preview',
      preview: {
        action: 'thread', author: '视觉楼主', boardId: board.id, boardName: board.name,
        boardTypeId: board.typeId, boardTypeName: board.typeName, boardTypePrompt: board.typePrompt,
        content: '这是论坛主楼预览正文，用于检查主楼与回复能在同一预览页显示。', draftId: null,
        mode: 'create', raw: '<forum><title>预览主题</title><content>这是论坛主楼预览正文。</content></forum>',
        replies: [{ author: '一楼', content: '第一条预览回复。' }, { author: '二楼', content: '第二条预览回复。' }],
        targetThreadId: '', targetVersionId: '', title: '论坛生成预览', warnings: [],
      },
      routeParams: { boardId: board.id }, title: '生成预览',
    });
    ...
    thread.generationRecord = context.createHiddenGenerationRecord('generate-thread', requirement, {
      boardId: board.id, boardName: board.name, boardTypeId: board.typeId,
      boardTypeName: board.typeName, boardTypePrompt: board.typePrompt,
    });
    context.resetPhoneToRoute('forum', 'generate-thread', '重写论坛主帖', { boardId: board.id, rewriteThreadId: thread.id });
    await context.waitForPaint();
  ...
  async function applyForumVersionScenario(versionScenario: string, scenarioContext: ForumGenerationScenarioContext) {
    ...
    const saved = forum.appendThreadVersion(board.id, thread.id, {
      ...
      content: interaction ? '采用这个主题版本后，应同时采用该版本的回复。' : '这是重新生成后的主题候选版本。主楼和回复属于同一个版本快照。',
    ...
    if (thread.content !== saved.version.content || JSON.stringify(thread.replies) !== JSON.stringify(candidateReplies)) {
      throw new Error('Forum rewrite version did not become the active thread snapshot');
    }
    if (!interaction && (thread.updatedAt === '2000-01-01T00:00:00.000Z' || board.updatedAt === '2000-01-01T00:00:00.000Z')) {
      throw new Error('Forum active rewrite version did not update ordering timestamps');
    }
    if (JSON.stringify(thread.replies) === originalReplies) throw new Error('Forum candidate replies were not distinct');
    scenarioContext.resetPhoneToRoute('forum', 'thread', saved.version.title, { boardId: board.id, threadId: thread.id, versionId: saved.version.id });
    await scenarioContext.waitForPaint();
    if (interaction) {
      if (document.querySelector('.pc-version-navigator .pc-primary-btn')) throw new Error('Forum version navigator still exposed a separate adoption action');
      if (thread.activeVersionId !== saved.version.id || JSON.stringify(thread.replies) !== JSON.stringify(candidateReplies)) throw new Error('Forum active version did not include its reply snapshot');
      const replyRewriteButton = Array.from(document.querySelectorAll<HTMLButtonElement>('.pc-reply-section button')).find(button => button.title.includes('重写回复'));
