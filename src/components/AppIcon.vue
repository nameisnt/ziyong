<template>
  ...
  <template v-else>
    <img
      v-for="(url, paper) in identityImages"
      :key="paper"
      :src="url"
      :class="['pc-app-identity-image', `pc-app-identity-image-${paper}`]"
      alt=""
    />
    <svg v-if="identityIcon" class="pc-svg-app-icon pc-identity-svg-app-icon" viewBox="0 0 24 24" aria-hidden="true">
...
import { getAppIdentityImageIcon } from '@/data/appIdentityImageIcons';
...
const identityImages = computed(() => {
  ...
  return getAppIdentityImageIcon(props.appId) ?? {};
});
