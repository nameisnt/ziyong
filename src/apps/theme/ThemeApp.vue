<template>
  <section class="pc-theme-app">
    <template v-if="view === 'root'">
      ...
      <button class="pc-section-card pc-theme-entry" type="button" @click="openView('icons')">
        ...
        <span class="pc-entry-copy">
          ...
          <small>{{ iconStyleLabel }} · {{ customizedAppCount ? `${customizedAppCount} 个单独调整` : t`整套设置` }}</small>
        </span>
      ...
      <section class="pc-section-card">
        ...
        <div v-if="selectedApp" class="pc-selected-icon-editor">
          <div class="pc-selected-app-head">
            <span :style="getAppPreviewStyle(selectedApp)"><i class="fa-solid" :class="getAppIcon(selectedApp)"></i></span>
            <div>
    ...
    <template v-else>
      ...
      <section class="pc-section-card">
        ...
        <label v-for="control in colorControls" :key="control.key" class="pc-color-row">
          ...
          <input :value="getColorInputValue(control.key)" type="color" @input="onVisualColorInput(control.key, $event)" />
        </label>
      </section>
        ...
        <label class="pc-select-field">
          ...
          <select :value="wallpaperSelectionValue" class="pc-select" @change="onWallpaperSelect">
            ...
            <optgroup :label="t`预设壁纸`">
              <option v-for="preset in WALLPAPER_PRESETS" :key="preset.id" :value="`preset:${preset.id}`">{{ preset.name }}</option>
            </optgroup>
            <optgroup v-if="settings.wallpaper.customWallpapers.length" :label="t`自定义壁纸`">
              <option v-for="wallpaper in settings.wallpaper.customWallpapers" :key="wallpaper.id" :value="`custom:${wallpaper.id}`">{{ wallpaper.name }}</option>
            </optgroup>
          </select>
        </label>
        <label class="pc-select-field">
          ...
          <select :value="fontSelectionValue" class="pc-select" @change="onFontSelect">
            ...
            <optgroup v-if="settings.customFont.fonts.length" :label="t`自定义字体`">
              <option v-for="font in settings.customFont.fonts" :key="font.id" :value="`custom:${font.id}`">{{ font.name }}</option>
            </optgroup>
          </select>
        </label>
        <label class="pc-select-field">
          ...
          <select :value="readerFontSelectionValue" class="pc-select" @change="onReaderFontSelect">
            ...
            <option v-for="option in fontOptions.filter(item => item.value)" :key="option.value" :value="option.value">{{ option.label }}</option>
            <optgroup v-if="settings.customFont.fonts.length" :label="t`自定义字体`">
              <option v-for="font in settings.customFont.fonts" :key="font.id" :value="`custom:${font.id}`">{{ font.name }}</option>
            </optgroup>
          </select>
    ...
    <input ref="iconInputEl" class="pc-hidden-input" type="file" accept="application/json,.json" @change="onIconPackSelected" />
    <input ref="themeInputEl" class="pc-hidden-input" type="file" accept="application/json,.json" @change="onThemeSelected" />
  </section>
</template>
...
type ColorKey = keyof Pick<VisualTheme, 'accentColor' | 'appIconBackgroundColor' | 'appIconColor' | 'backgroundColor' | 'dockColor' | 'surfaceColor'>;
...
const themePresets: ThemePreset[] = [
  {
    id: 'clear', name: '清透', mode: 'light', accentColor: '#007aff', appIconBackgroundColor: '#ffffff', appIconColor: '',
    backgroundColor: '#f2f2f7', borderColor: 'rgba(28, 28, 30, 0.08)', cardRadius: 20, controlRadius: 16,
    dangerColor: '#ff5a5f', dockActiveColor: '#007aff', dockColor: 'rgba(255, 255, 255, 0.82)', hintColor: '#2d9cdb',
    iconRadius: 14, mutedTextColor: '#6d6d74', primaryTextColor: '#ffffff', softButtonColor: 'rgba(255, 255, 255, 0.96)',
    surfaceColor: 'rgba(255, 255, 255, 0.82)', surfaceStrongColor: 'rgba(255, 255, 255, 0.96)', textColor: '#1c1c1e',
  },
  {
    id: 'soft', name: '柔和', mode: 'light', accentColor: '#ef476f', appIconBackgroundColor: '#fff8fa', appIconColor: '',
    backgroundColor: '#fff1f5', borderColor: 'rgba(160, 45, 78, 0.12)', cardRadius: 22, controlRadius: 18,
    dangerColor: '#e0314f', dockActiveColor: '#ef476f', dockColor: 'rgba(255, 246, 249, 0.86)', hintColor: '#b846d6',
    iconRadius: 16, mutedTextColor: '#7d5964', primaryTextColor: '#ffffff', softButtonColor: 'rgba(255, 248, 250, 0.98)',
    surfaceColor: 'rgba(255, 249, 251, 0.84)', surfaceStrongColor: 'rgba(255, 255, 255, 0.98)', textColor: '#2b171d',
  },
  {
    id: 'mint', name: '薄荷', mode: 'light', accentColor: '#2a9d8f', appIconBackgroundColor: '#f8fffd', appIconColor: '',
    backgroundColor: '#edf8f5', borderColor: 'rgba(28, 102, 94, 0.13)', cardRadius: 18, controlRadius: 14,
    dangerColor: '#d94f62', dockActiveColor: '#2a9d8f', dockColor: 'rgba(246, 255, 252, 0.86)', hintColor: '#268bd2',
    iconRadius: 12, mutedTextColor: '#526b68', primaryTextColor: '#ffffff', softButtonColor: 'rgba(248, 255, 253, 0.98)',
    surfaceColor: 'rgba(248, 255, 253, 0.84)', surfaceStrongColor: 'rgba(255, 255, 255, 0.98)', textColor: '#142421',
  },
  {
    id: 'midnight', name: '午夜', mode: 'dark', accentColor: '#a78bfa', appIconBackgroundColor: 'rgba(255, 255, 255, 0.12)', appIconColor: '',
    backgroundColor: '#17151d', borderColor: 'rgba(237, 233, 254, 0.12)', cardRadius: 20, controlRadius: 16,
    dangerColor: '#ff6b81', dockActiveColor: '#c4b5fd', dockColor: 'rgba(38, 34, 48, 0.84)', hintColor: '#67d4ff',
    iconRadius: 14, mutedTextColor: '#b8b1c4', primaryTextColor: '#17151d', softButtonColor: 'rgba(255, 255, 255, 0.12)',
    surfaceColor: 'rgba(255, 255, 255, 0.07)', surfaceStrongColor: 'rgba(255, 255, 255, 0.12)', textColor: '#f7f5fa',
  },
  {
    id: 'ocean', name: '深海', mode: 'dark', accentColor: '#38bdf8', appIconBackgroundColor: 'rgba(56, 189, 248, 0.12)', appIconColor: '',
    backgroundColor: '#071923', borderColor: 'rgba(186, 230, 253, 0.12)', cardRadius: 18, controlRadius: 14,
    dangerColor: '#fb7185', dockActiveColor: '#38bdf8', dockColor: 'rgba(12, 39, 52, 0.88)', hintColor: '#22d3ee',
    iconRadius: 12, mutedTextColor: '#9fb8c4', primaryTextColor: '#06202d', softButtonColor: 'rgba(186, 230, 253, 0.11)',
    surfaceColor: 'rgba(186, 230, 253, 0.07)', surfaceStrongColor: 'rgba(186, 230, 253, 0.11)', textColor: '#edfaff',
  },
  {
    id: 'graphite', name: '石墨', mode: 'dark', accentColor: '#f5b942', appIconBackgroundColor: 'rgba(255, 255, 255, 0.11)', appIconColor: '',
    backgroundColor: '#18191b', borderColor: 'rgba(255, 255, 255, 0.1)', cardRadius: 16, controlRadius: 12,
    dangerColor: '#ff6b6b', dockActiveColor: '#f5b942', dockColor: 'rgba(38, 39, 42, 0.9)', hintColor: '#66c7f2',
    iconRadius: 10, mutedTextColor: '#a9aaad', primaryTextColor: '#1a1b1d', softButtonColor: 'rgba(255, 255, 255, 0.1)',
    surfaceColor: 'rgba(255, 255, 255, 0.06)', surfaceStrongColor: 'rgba(255, 255, 255, 0.1)', textColor: '#f5f5f5',
  },
  {
    id: 'velvet', name: '绒夜', mode: 'dark', accentColor: '#f472b6', appIconBackgroundColor: 'rgba(255, 255, 255, 0.1)', appIconColor: '',
    backgroundColor: '#25131f', borderColor: 'rgba(253, 242, 248, 0.12)', cardRadius: 22, controlRadius: 18,
    dangerColor: '#fb7185', dockActiveColor: '#f9a8d4', dockColor: 'rgba(65, 29, 49, 0.88)', hintColor: '#c084fc',
    iconRadius: 16, mutedTextColor: '#d0aebb', primaryTextColor: '#2a1321', softButtonColor: 'rgba(255, 255, 255, 0.11)',
    surfaceColor: 'rgba(255, 255, 255, 0.07)', surfaceStrongColor: 'rgba(255, 255, 255, 0.11)', textColor: '#fff4f8',
  },
  {
    id: 'cypress', name: '青岚', mode: 'dark', accentColor: '#34d399', appIconBackgroundColor: 'rgba(209, 250, 229, 0.11)', appIconColor: '',
    backgroundColor: '#10241f', borderColor: 'rgba(209, 250, 229, 0.12)', cardRadius: 18, controlRadius: 14,
    dangerColor: '#fb7185', dockActiveColor: '#6ee7b7', dockColor: 'rgba(19, 55, 45, 0.88)', hintColor: '#67e8f9',
    iconRadius: 12, mutedTextColor: '#a7c6bb', primaryTextColor: '#102820', softButtonColor: 'rgba(209, 250, 229, 0.11)',
    surfaceColor: 'rgba(209, 250, 229, 0.07)', surfaceStrongColor: 'rgba(209, 250, 229, 0.11)', textColor: '#effff8',
  },
  {
    id: 'sky', name: '晴空', mode: 'light', accentColor: '#0284c7', appIconBackgroundColor: '#f7fcff', appIconColor: '',
    backgroundColor: '#eaf6ff', borderColor: 'rgba(2, 132, 199, 0.12)', cardRadius: 18, controlRadius: 14,
    dangerColor: '#e4546f', dockActiveColor: '#0284c7', dockColor: 'rgba(248, 253, 255, 0.88)', hintColor: '#0ea5e9',
    iconRadius: 12, mutedTextColor: '#577181', primaryTextColor: '#ffffff', softButtonColor: 'rgba(248, 253, 255, 0.98)',
    surfaceColor: 'rgba(248, 253, 255, 0.84)', surfaceStrongColor: 'rgba(255, 255, 255, 0.98)', textColor: '#102634',
  },
];
...
const iconOptions = [
  'fa-note-sticky', 'fa-book', 'fa-feather-pointed', 'fa-comments', 'fa-masks-theater', 'fa-envelope-open-text',
  'fa-bookmark', 'fa-wand-magic-sparkles', 'fa-chart-column', 'fa-folder-open', 'fa-glasses', 'fa-sliders',
  'fa-diagram-project', 'fa-address-card', 'fa-code', 'fa-image', 'fa-photo-film', 'fa-highlighter',
  'fa-clock', 'fa-calendar-days', 'fa-heart', 'fa-star', 'fa-user', 'fa-users', 'fa-house', 'fa-pen',
  'fa-scroll', 'fa-music', 'fa-video', 'fa-camera-retro', 'fa-plus', 'fa-palette',
];
...
const builtinIconPacks: BuiltinIconPack[] = [
  ...
  },
  {
    ...
    icons: {
      archive: 'fa-folder-open', bagu: 'fa-highlighter', 'chat-insert': 'fa-pen', comfy: 'fa-image',
      diary: 'fa-book', digest: 'fa-note-sticky', extras: 'fa-feather-pointed',
      favorites: 'fa-bookmark', forum: 'fa-comments', gallery: 'fa-photo-film', games: 'fa-masks-theater', letters: 'fa-envelope-open-text',
      media: 'fa-image', music: 'fa-music', 'preset-manager': 'fa-wand-magic-sparkles', profiles: 'fa-address-card', prompts: 'fa-quote-left',
      reader: 'fa-glasses', 'regex-display': 'fa-highlighter', relationship: 'fa-users', 'scene-planner': 'fa-pen', settings: 'fa-sliders',
      'storylines': 'fa-book-open', stats: 'fa-chart-column', summary: 'fa-note-sticky', theater: 'fa-masks-theater', theme: 'fa-palette',
      timekeeper: 'fa-clock', tutorial: 'fa-book-open', video: 'fa-video', workbench: 'fa-pen', 'world-slots': 'fa-book', 'worldbook-link': 'fa-book-open-reader',
    },
  ...
  },
  {
    ...
    icons: {
      archive: 'fa-folder-open', bagu: 'fa-filter-circle-xmark', 'chat-insert': 'fa-plus', comfy: 'fa-image',
      diary: 'fa-calendar-days', digest: 'fa-highlighter', extras: 'fa-file-pen',
      favorites: 'fa-star', forum: 'fa-table-columns', gallery: 'fa-images', games: 'fa-gamepad', letters: 'fa-inbox',
      media: 'fa-photo-film', music: 'fa-music', 'preset-manager': 'fa-sliders', profiles: 'fa-address-card', prompts: 'fa-wand-magic-sparkles',
      reader: 'fa-list', 'regex-display': 'fa-code', relationship: 'fa-users', 'scene-planner': 'fa-diagram-project', settings: 'fa-gear',
      'storylines': 'fa-list-check', stats: 'fa-chart-line', summary: 'fa-list-check', theater: 'fa-clapperboard', theme: 'fa-palette',
      timekeeper: 'fa-calendar-days', tutorial: 'fa-circle-question', video: 'fa-video', workbench: 'fa-table-columns', 'world-slots': 'fa-table-cells-large', 'worldbook-link': 'fa-book-open-reader',
    },
  ...
  },
  {
    ...
    icons: {
      archive: 'fa-box-archive', bagu: 'fa-wand-magic-sparkles', 'chat-insert': 'fa-comment-dots', comfy: 'fa-camera-retro',
      diary: 'fa-book-open', digest: 'fa-bookmark', extras: 'fa-scroll',
      favorites: 'fa-heart', forum: 'fa-comment-dots', gallery: 'fa-images', games: 'fa-gamepad', letters: 'fa-envelope',
      media: 'fa-film', music: 'fa-headphones', 'preset-manager': 'fa-sliders', profiles: 'fa-user', prompts: 'fa-wand-magic-sparkles',
      reader: 'fa-glasses', 'regex-display': 'fa-code', relationship: 'fa-users', 'scene-planner': 'fa-clapperboard', settings: 'fa-gear',
      'storylines': 'fa-film', stats: 'fa-chart-column', summary: 'fa-note-sticky', theater: 'fa-masks-theater', theme: 'fa-palette',
      timekeeper: 'fa-clock', tutorial: 'fa-circle-play', video: 'fa-video', workbench: 'fa-photo-film', 'world-slots': 'fa-images', 'worldbook-link': 'fa-book-open-reader',
    },
...
const customizedAppCount = computed(() => new Set([
  ...Object.keys(settings.value.visualTheme.appIconOverrides),
  ...Object.keys(settings.value.visualTheme.appAccentOverrides),
]).size);
const selectedCustomFont = computed(() => settings.value.customFont.fonts.find(item => settings.value.fontFamily === settingsStore.getCustomFontFamily(item.id)) ?? null);
const readerSelectedCustomFont = computed(() => settings.value.customFont.fonts.find(item => settings.value.reader.fontFamily === settingsStore.getCustomFontFamily(item.id)) ?? null);
const fontSelectionValue = computed(() => selectedCustomFont.value ? `custom:${selectedCustomFont.value.id}` : settings.value.fontFamily);
const readerFontSelectionValue = computed(() => readerSelectedCustomFont.value ? `custom:${readerSelectedCustomFont.value.id}` : settings.value.reader.fontFamily);
const selectedCustomWallpaper = computed(() => {
  if (settings.value.wallpaper.mode !== 'custom') return null;
  return settings.value.wallpaper.customWallpapers.find(item => item.id === settings.value.wallpaper.selectedCustomId)
...
const wallpaperSelectionValue = computed(() => {
  ...
  if (settings.value.wallpaper.mode === 'custom') return `custom:${selectedCustomWallpaper.value?.id || settings.value.wallpaper.selectedCustomId}`;
  return 'none';
});
const activePresetLabel = computed(() => {
  const matched = visibleThemePresets.value.find(preset => (
...
const appearanceSummary = computed(() => {
  const wallpaper = settings.value.wallpaper.mode === 'preset'
    ? getWallpaperPreset(settings.value.wallpaper.presetId)?.name || '预设壁纸'
    : settings.value.wallpaper.mode === 'custom' ? selectedCustomWallpaper.value?.name || '自定义壁纸' : '默认背景';
  const font = fontOptions.find(item => item.value === settings.value.fontFamily)?.label || selectedCustomFont.value?.name || '自定义字体';
  return `${wallpaper} · ${font}`;
});
const previewStyle = computed(() => {
  const wallpaper = settings.value.wallpaper.mode === 'preset' ? getWallpaperPreset(settings.value.wallpaper.presetId)?.background : '';
  const customPath = selectedCustomWallpaper.value?.path || settings.value.wallpaper.customPath;
  const backgroundImage = settings.value.wallpaper.mode === 'custom' && customPath
    ? `url("/${encodeURI(customPath.replace(/^\/+/, ''))}")`
    : wallpaper || 'none';
  return {
    backgroundColor: settings.value.visualTheme.backgroundColor || (settings.value.theme === 'dark' ? '#1c1c1e' : '#f2f2f7'),
    backgroundImage,
...
function isThemePackActive(pack: ThemePack) {
  ...
  return light.accentColor.toLowerCase() === pack.light.accentColor.toLowerCase()
...
function getAppPreviewStyle(app: (typeof PHONE_APPS)[number]) {
  ...
  return {
    ...
    '--preview-app-bg': settings.value.visualTheme.appIconBackgroundColor || `color-mix(in srgb, ${accent} 18%, var(--pc-surface-strong) 82%)`,
  };
}
...
function haveSameIconOverrides(left: Record<string, string>, right: Record<string, string>) {
  ...
  return leftEntries.length === rightEntries.length
    && leftEntries.every(([appId, icon]) => right[appId] === icon);
}

function isBuiltinIconPackActive(pack: BuiltinIconPack) {
  return haveSameIconOverrides(settings.value.themeProfiles.light.visualTheme.appIconOverrides, pack.icons)
...
function onReaderFontSelect(event: Event) {
  const value = (event.target as HTMLSelectElement).value;
  if (value.startsWith('custom:')) settingsStore.setReaderFontFamily(settingsStore.getCustomFontFamily(value.slice('custom:'.length)));
...
async function resetCurrentTheme() {
  const confirmed = await phone.confirmNotice(`恢复${settings.value.theme === 'light' ? '日间' : '夜间'}方案的默认外观吗？`, {
    confirmLabel: '恢复默认',
    title: '恢复主题',
  });
  if (!confirmed) return;
...
function readIconPack(input: unknown) {
  ...
  const source = record.appIconOverrides ?? record.icons ?? (record.visualTheme && typeof record.visualTheme === 'object'
    ? (record.visualTheme as Record<string, unknown>).appIconOverrides : null);
...
async function onThemeSelected(event: Event) {
  ...
  try {
    const parsed = JSON.parse(await file.text()) as {
      fontFamily?: unknown; readerFontFamily?: unknown; visualTheme?: unknown;
    ...
    else if (parsed.wallpaper?.mode === 'preset' && typeof parsed.wallpaper.presetId === 'string') await settingsStore.selectWallpaperPreset(parsed.wallpaper.presetId);
...
.pc-style-icons.style-native i:nth-child(2) { color: #34a853; }
.pc-style-icons.style-native i:nth-child(3) { color: #ef476f; }
.pc-style-icons.style-unified i { background: var(--pc-theme-accent); color: var(--pc-primary-text); }
...
@media (max-width: 380px) {
  .pc-preview-icons { grid-template-columns: repeat(5, minmax(0, 1fr)); }
  .pc-preview-icons > :last-child { display: none; }
  .pc-app-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
  .pc-icon-picker { grid-template-columns: repeat(7, minmax(0, 1fr)); }
}
</style>
