  ...
  <section class="pc-profiles-page">
    <article class="pc-editor-card">
      ...
      <input v-model="title" class="pc-field" type="text" placeholder="标题" /><select
        ...
        @change="$emit('change-table')"
      >
        <option v-for="table in tables" :key="table.id" :value="table.id">{{ table.name }}</option></select
      ><input
        v-if="summaryEnabled"
        v-model="summary"
        class="pc-field"
      ...
      /><template v-for="column in columns" :key="column.id"
        ><label class="pc-field-group"
          ...
          ><textarea v-if="column.type === 'textarea'" v-model="fields[column.id]" class="pc-area compact"></textarea
          ><select
          ...
          ><input
            ...
            class="pc-field"
            type="text"
            :placeholder="column.type === 'tags' ? '用逗号分隔' : '可留空'" /></label
      ></template>
