<template>
  <section class="pc-preset-page pc-preset-editor-page">
    <article class="pc-editor-card pc-preset-editor">
      ...
      <label class="pc-field-group"><span class="pc-field-label">副本名称</span><input v-model="name" class="pc-field" type="text" placeholder="副本名称" /></label>
      <label class="pc-field-group">
        <span class="pc-field-label">消息角色</span>
        <select v-model="role" class="pc-field pc-select">
          <option value="system">系统</option><option value="user">用户</option><option value="assistant">AI</option>
        </select>
      </label>
      <textarea v-model="content" class="pc-area" placeholder="预设条目内容"></textarea>
      <label class="pc-preset-filter-row">
        <span>保存后立即启用副本</span>
        <span class="pc-toggle" title="保存后立即启用副本"><input v-model="enabled" type="checkbox" aria-label="保存后立即启用副本" /><span aria-hidden="true"></span></span>
      </label>
      <div class="pc-form-actions">
        <button class="pc-soft-btn" type="button" :disabled="saving" @click="$emit('back')">取消</button>
        <button class="pc-primary-btn" type="button" :disabled="saving || !name.trim()" @click="$emit('save')">{{ saving ? '保存中' : '保存副本' }}</button>
      </div>
    </article>
  </section>
</template>
