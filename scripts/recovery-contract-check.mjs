...
assert.match(model, /createSimilarBackupGroups/);
assert.match(model, /getBackupMessageSimilarity/);
...
assert.match(store, /deleteSimilarBackups/);
...
assert.match(maintenanceFlow, /同时查找完全相同、严格续长和逐楼内容至少 90% 相同的备份/);
assert.match(maintenanceFlow, /每组固定保护备份时间最新的一份/);
assert.match(maintenanceFlow, /删除前会重新下载复核/);
