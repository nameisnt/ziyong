...
const sources = await Promise.all(
  [...expectedCounts].map(async ([name, expectedCount]) => ({
    expectedCount,
    file: `src/apps/app-builder/${name}`,
    source: await readFile(new URL(`../../src/apps/app-builder/${name}`, import.meta.url), 'utf8'),
  })),
);

test('every app builder icon button has the same accessible name as its title', () => {
