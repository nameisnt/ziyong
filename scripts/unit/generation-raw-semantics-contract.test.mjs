...
test('generation keeps the complete model output while parsing a cleaned temporary input', () => {
  ...
  assert.match(serviceSource, /const rawOutput = cleanedOutput\.originalOutput;\s*const parseInput = cleanedOutput\.content;/);
...
test('task and backup migrations distinguish legacy raw records from new complete outputs', () => {
  ...
  assert.match(backupSource, /key:\s*'generation-tasks'[\s\S]*?migrateImport:\s*migrateGenerationTasksBackupData[\s\S]*?schemaVersion:\s*3/);
