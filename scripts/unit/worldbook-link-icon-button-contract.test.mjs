...
const expectedCounts = new Map([
  ...
  ['WorldbookDetailPage.vue', 3],
  ['WorldbookEntryEditorPage.vue', 1],
]);
const sources = await Promise.all(
  [...expectedCounts].map(async ([name, expectedCount]) => ({
    expectedCount,
    file: `src/apps/worldbook-link/pages/${name}`,
    source: await readFile(new URL(`../../src/apps/worldbook-link/pages/${name}`, import.meta.url), 'utf8'),
  })),
);

test('every Worldbook Link icon button has the same accessible name as its title', () => {
