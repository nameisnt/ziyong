...
const file = 'src/apps/music/MusicApp.vue';
const source = await readFile(new URL('../../src/apps/music/MusicApp.vue', import.meta.url), 'utf8');

test('every Music icon button has the same accessible name as its title', () => {
  ...
  assert.equal(iconButtons.length, 5, file);
  ...
  for (const tag of iconButtons) {
    ...
    assert.deepEqual(ariaLabel.slice(1), title.slice(1), `${file}: ${tag}`);
  }
});
