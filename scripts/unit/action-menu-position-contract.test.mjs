...
const movedCandidates = [
  ['src/components/extras/ExtrasBookOverviewPage.vue', '管理'],
  ['src/components/diary/DiaryBookPage.vue', '管理'],
  ['src/components/summary/SummaryBookPage.vue', '管理'],
...
test('directory management menus precede context copy while immediate actions stay trailing', async () => {
  for (const path of [
    'src/components/extras/ExtrasBookOverviewPage.vue',
    'src/components/diary/DiaryBookPage.vue',
    'src/components/summary/SummaryBookPage.vue',
  ]) {
    const source = await read(path);
...
test('single-action and creation menus are not mechanically moved into the left management slot', async () => {
  const letters = await read('src/components/letters/LettersBookPage.vue');
  const theater = await read('src/components/theater/TheaterHistoryPage.vue');
  assert.doesNotMatch(letters, /<ActionMenu[^>]*icon-only[^>]*管理/u);
  assert.doesNotMatch(theater, /<ActionMenu[^>]*icon-only[^>]*管理/u);

  for (const path of [
    'src/components/forum/ForumBoardPage.vue',
