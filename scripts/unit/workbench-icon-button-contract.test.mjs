...
const file = 'src/apps/workbench/WorkbenchApp.vue';
const source = await readFile(new URL('../../src/apps/workbench/WorkbenchApp.vue', import.meta.url), 'utf8');

test('every Workbench icon button has the same accessible name as its title', () => {
  ...
  assert.equal(iconButtons.length, 8, file);
