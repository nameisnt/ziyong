...
const file = 'src/components/theater/TheaterHistoryPage.vue';
const source = await readFile(new URL('../../src/components/theater/TheaterHistoryPage.vue', import.meta.url), 'utf8');

test('every Theater history icon button has the same accessible name as its title', () => {
  ...
  const iconButtons = [...source.matchAll(/<button\b(?:[^>"']|"[^"]*"|'[^']*')*>/g)]
    .map(match => match[0])
    .filter(tag => /(?:\bclass="[^"]*\bpc-icon-btn\b[^"]*"|\b:class="[^"]*'pc-icon-btn'[^"]*")/.test(tag));
