...
const readerScenarios = ['reader-detail', 'reader-theme-appearance', 'reader-footer-persistence', 'reader-catalog'];

test('reader visual scenarios have one dedicated module and do not keep branches in the coordinator', async () => {
  const [harness, readerModule] = await Promise.all([readFile(harnessUrl, 'utf8'), readFile(readerModuleUrl, 'utf8')]);
