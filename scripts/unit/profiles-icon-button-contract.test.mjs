...
const expectedCounts = new Map([
  ['pages/ProfilesCatalogPage.vue', 3],
  ['pages/ProfilesTableEditorPage.vue', 2],
  ['pages/ProfilesTableManagerPage.vue', 1],
]);
const sources = await Promise.all(
  [...expectedCounts].map(async ([name, expectedCount]) => ({
    expectedCount,
    file: `src/apps/profiles/${name}`,
    source: await readFile(new URL(`../../src/apps/profiles/${name}`, import.meta.url), 'utf8'),
  })),
);

test('every Profiles catalog and table icon button has the same accessible name as its title', () => {
