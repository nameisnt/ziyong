...
43. 批量生成先进入逐项预览再保存；失败草稿保留原始输出，可编辑、重新解析或按 replay 重新生成。
44. 首页生成任务区始终显示、内部滚动且保留清理按钮；移动端首页手势与 Dock 五槽容量按真实布局处理。
...
47. 世界书槽位根页使用管理菜单承载新增和同步；编辑页启用开关位于名称右侧，正文输入区使用高文本域；世界书联动条目可复制为新条目，也可复制为小剧场类型。
48. 酒馆备份查重分为完全相同、严格续长和至少 90% 逐楼相同三类，候选均可取消选择且删除前重新复核。
49. `macro-builder` 生成插件私有 `{{phone:...}}` 宏并只在手机生成期间注册；`status-display` 只投影当前聊天，正则方案读取最新命中 AI 原文，MVU 方案读取 `stat_data`，结果不得另存为内容条目。
