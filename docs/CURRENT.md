...
44. 全局手机顶栏使用紧凑高度；界面采用内置纸张纹理与平面纸面层级，不提供壁纸；首页生成任务区始终显示并内部滚动，“清理”只移除已完成且没有失败草稿的通知记录，不删除保存内容或预览；设置页使用可横向滚动的平面分类栏和纸面列表，不再提供旧分页主页的行数/每页容量入口；首页默认分为阅读与记录、生成、内容整理、预设与世界书、酒馆工具、资料关系、插件工具、小游戏，聊天档案独立，设置/提示词/收藏/教程固定在 Dock；普通 App 的页面外边距只由 `.pc-screen` 提供。
