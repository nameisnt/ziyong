...
44. 全局手机顶栏、页面边距和搜索框使用紧凑尺寸；界面采用内置纸张纹理与平面纸面层级，浅色默认 A4、深色默认黑色卡纸，不提供壁纸；首页按“当前聊天 → 固定生成任务区 → App 搜索 → 横向分组标签 → 当前分组完整 App 网格 → Dock”排列，不渲染分页、文件夹预览卡或独立 App 区；默认分组为阅读与记录、生成、内容整理、预设与世界书、酒馆工具、资料关系、插件工具、小游戏，未归类的新 App 自动进入插件工具；聊天档案/收藏/提示词/教程/设置固定在五项 Dock；旧布局一次迁移到 layout v4，不保留旧分组、独立项或 Dock 配置；返回主页只恢复来源分组，不自动打开管理弹窗；任务“清理”只移除已完成且没有失败草稿的通知记录，不删除保存内容或预览；普通 App 的页面外边距只由 `.pc-screen` 提供；设置页使用单行分类选择器，分为界面、阅读、生成、连接、数据和高级，外部 API 配置进入二级编辑页，简单设置行不得重复显示当前值和同义说明。
